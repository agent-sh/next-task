...
/**
 * Build cached free-text options for a question.
 * These are user-entered responses from previous runs.
 * @param {string} questionKey - 'source' | 'priority' | 'stopPoint'
 * @returns {Array<Object>} Options array for AskUserQuestion
 */
function buildFreeTextOptions(questionKey) {
  const cached = sourceCache.getFreeTextOptions(questionKey);
  return cached.map(opt => ({
    label: opt.label,
    description: `Custom (used ${opt.useCount}x)`
  }));
}
    ...
    questions: [
      ...
      },
      {
        ...
        options: [
          ...
          { label: 'Other', description: 'Describe your priority filter' }
        ],
        multiSelect: false
      },
      {
        ...
        options: [
          ...buildFreeTextOptions('stopPoint'),
          { label: 'Merged', description: 'Until PR is merged to default branch' },
          ...
          { label: 'Other', description: 'Describe your stopping point' }
        ],
        multiSelect: false
      }
    ],
...
function parseAndCachePolicy(responses) {
  ...
  // Cache free-text selections for priority and stopPoint
  cacheFreeTextSelection('priority', responses.priority);
  cacheFreeTextSelection('stopPoint', responses.stopPoint);

  return policy;
}

/**
 * Known (built-in) option labels per question.
 * Anything not in this set is considered free-text.
 */
const KNOWN_OPTIONS = {
  priority: ['All', 'Bugs', 'Security', 'Features', 'Other'],
  stopPoint: ['Merged', 'PR Created', 'Implemented', 'Deployed', 'Production', 'Other']
};

/**
 * Cache a free-text selection if it's not a known option.
 * Also decay other cached options for this question.
 * @param {string} questionKey - 'priority' | 'stopPoint'
 * @param {string} selection - The user's selection text
 */
function cacheFreeTextSelection(questionKey, selection) {
  if (!selection) return;
  const known = KNOWN_OPTIONS[questionKey] || [];

  // Check if this is a known option or a cached free-text label
  const cached = sourceCache.getFreeTextOptions(questionKey);
  const isCachedOption = cached.some(o => o.label === selection);
  const isKnownOption = known.includes(selection);

  if (isCachedOption) {
    const opt = cached.find(o => o.label === selection);
    sourceCache.trackFreeTextSelection(questionKey, opt.value, selection);
    sourceCache.decayFreeTextOptions(questionKey, opt.value);
  } else if (!isKnownOption && selection !== 'Other') {
    // New free-text entry
    sourceCache.trackFreeTextSelection(questionKey, selection, selection);
    sourceCache.decayFreeTextOptions(questionKey, selection);
  } else {
    // Known option selected - decay all free-text options
    sourceCache.decayFreeTextOptions(questionKey, null);
  }
}
...
function mapPriority(selection) {
  ...
  return map[selection] || selection || 'all';
}
...
function mapStopPoint(selection) {
  ...
  return map[selection] || selection || 'merged';
}
